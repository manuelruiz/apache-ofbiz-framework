    ...
    public void checkRhsType(ModelEntity modelEntity, Delegator delegator) {
        ...
        // make sure the type matches the field Java type
            if (!ObjectType.instanceOf(value, type.getJavaType())) {
            	// Alberto: byte[] class is now String, we avoid this validation to make field usable
            	if (type.getJavaType().equals("byte[]")) {
            		Debug.log("Convert byte[] to String type, only in PostgreSQL!", module);
            	} else {
